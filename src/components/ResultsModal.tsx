  ...
  const handlePrintResults = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>${tool.title} - Assessment Results</title>
            <style>
              body { font-family: Arial, sans-serif; margin: 20px; }
              .header { text-align: center; margin-bottom: 30px; }
              .risk-level { background: #f0f0f0; padding: 15px; border-radius: 8px; margin: 20px 0; }
              .recommendations { margin: 20px 0; }
              .recommendations ul { list-style-type: disc; padding-left: 20px; }
              .disclaimer { background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 8px; margin-top: 20px; }
              .score { text-align: center; font-size: 18px; margin: 20px 0; }
            </style>
          </head>
          <body>
            <div class="header">
            ...
            <div class="score">
              <strong>Your Score: ${score} out of ${maxScore} (${Math.round(percentage)}%)</strong>
            </div>
            
            <div class="risk-level">
              <h3>Risk Level: ${riskAssessment.level}</h3>
              <p>${getResultMessage(tool.id, riskAssessment.level)}</p>
            </div>
            ...
            <div class="disclaimer">
              ...
              <p>This assessment is for informational purposes only and is not a substitute for professional medical advice. Always consult with a qualified healthcare provider for proper diagnosis and treatment.</p>
            </div>
          </body>
        </html>
      `);
