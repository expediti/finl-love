const handlePrintResults = ({
  ...
  if (printWindow) {
    printWindow.document.write(`
      <html>
        <head>
          <title>${tool?.title || "Assessment"} - Assessment Results - FitScan</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 0; padding: 20px; line-height: 1.6; }
            .print-header { text-align: center; margin-bottom: 30px; border-bottom: 3px solid #3b82f6; padding-bottom: 20px; }
            .logo-section { display: flex; align-items: center; justify-content: center; margin-bottom: 15px; }
            .logo-img { width: 60px; height: 60px; margin-right: 15px; border-radius: 8px; object-fit: contain; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
            .site-name { font-size: 28px; font-weight: bold; color: #1f2937; margin: 0; }
            .site-tagline { font-size: 14px; color: #6b7280; margin: 5px 0 0 0; }
            .assessment-title { font-size: 24px; color: #374151; margin: 20px 0 10px 0; }
            .print-date { font-size: 12px; color: #9ca3af; margin: 0; }
            .risk-level { background: #f9fafb; border: 2px solid #e5e7eb; padding: 20px; border-radius: 12px; margin: 25px 0; text-align: center; }
            .risk-high { border-color: #ef4444; background: #fef2f2; }
            .risk-moderate, .risk-mild { border-color: #f59e0b; background: #fffbeb; }
            .risk-low { border-color: #10b981; background: #f0fdf4; }
            .score-section { text-align: center; font-size: 18px; margin: 25px 0; background: #f8fafc; padding: 15px; border-radius: 8px; }
            .recommendations { margin: 25px 0; }
            .recommendations h3 { color: #374151; border-bottom: 2px solid #e5e7eb; padding-bottom: 8px; }
            .recommendations ul { list-style-type: none; padding-left: 0; }
            .recommendations li { background: #f8fafc; margin: 8px 0; padding: 12px; border-left: 4px solid #3b82f6; border-radius: 4px; }
            .disclaimer { background: #fff3cd; border: 2px solid #ffeaa7; padding: 20px; border-radius: 12px; margin-top: 30px; }
        ...
        <body>
          <div class="print-header">
            ...
            <h2 class="assessment-title">${tool?.title || "Health Assessment"}</h2>
            <h3 style="color: #374151; margin: 10px 0;">Assessment Results</h3>
            <p class="print-date">Generated on: ${new Date().toLocaleString('en-US', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
              day: 'numeric',
          ...
          <div class="recommendations">
            ...
            <ul>
              ${recommendations?.map(rec => `<li>• ${rec}</li>`).join('') || '<li>No recommendations found.</li>'}
            </ul>
          </div>
